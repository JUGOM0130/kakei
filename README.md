...
- **保有一覧**: 保有株数・平均取得単価・取得額・現在値・評価損益。**株価は Yahoo!ファイナンス (非公式 API) から自動取得** (ページを開くと自動更新+「株価更新」ボタン、取得時点表示)。取得できない銘柄のみ手動入力にフォールバック
- **ウォッチリスト (目標価格)**: 「この価格まで下がったら買い / 上がったら売り」を銘柄ごとに登録。現在値と比較して達成すると「買いチャンス!」表示、ホームにも到達カードを表示。目標の変更・メモ可
- **決算月の管理**: 銘柄ごとに決算月を設定 (保有・ウォッチ両方から設定可、銘柄単位で共有)。今月が決算月の銘柄をホームに表示
- **履歴の絞り込み検索**: 銘柄名・コード・証券会社のテキスト検索 + 買付/売却 + 口座区分で絞り込み (件数表示付き)
...
| GET | `/api/stocks/positions/` | (KABU) 保有ポジション (移動平均) + 評価損益 + 決算月 |
| CRUD | `/api/stocks/watches/` | (KABU) 目標価格ウォッチ (現在値・達成判定付き) |
| POST | `/api/stocks/prices/refresh/` | (KABU) 保有+ウォッチ銘柄の株価を Yahoo!ファイナンスから取得 |
| PUT | `/api/stocks/info/{code}/` | (KABU) 銘柄の決算月を設定 |
