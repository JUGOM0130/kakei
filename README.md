...
- **明細 CSV の一括取込**: VPASS / 楽天カード e-NAVI からダウンロードした明細 CSV を選ぶだけで、請求(親)+内訳として一括登録。文字コード (Shift_JIS/UTF-8) と列構成は自動判定 (失敗時は手動割り当て)。行ごとに取込対象・カテゴリ・共有を選択でき、**店名→カテゴリ/共有は学習**されて次回から自動プリセット。二重取込の注意表示あり (履歴画面 →「CSV取込」)
